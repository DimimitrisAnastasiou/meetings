...
def main():
    ...
    initial_radii = [8.0]
    tensions = np.arange(0.5,3., 0.5)

    with open("directories_surf.txt", "w") as directories:
        directories.write("")

    with open("directories_surf_eq.txt", "w") as directories:
        directories.write("")

    with open("directories_surf_prod.txt", "w") as directories:
        directories.write("")

    # Create directories and generate simulation files
    print("RADIUS:",initial_radii)
    print("TENSIONS:",tensions)
    for radius in initial_radii:
        for tension in tensions:
            # lambdas = np.arange((radius*tensions[0])-3 if (radius*tensions[0])-3 > 0 else 0.5,(radius*tensions[-1])+3.5, 0.5)  # [1, 2, ..., 10]
            lambdas = np.linspace((radius*tensions[0])-3 if (radius*tensions[0])-3 > 0 else 0.5,(radius*tensions[-1])+3.5,20)  # [1, 2, ..., 10]
            print("LAMBDA:",lambdas ,"With tension: ", tension)
            for lamda in lambdas:

                surf_sim_dir = os.path.join(base_dir, f"radius_{radius:.2f}",f"lambda_{lamda:.2f}",f"tau_{tension:.2f}","surf")
                os.makedirs(surf_sim_dir, exist_ok=True)

                surf_sim_dir_eq = os.path.join(base_dir, f"radius_{radius:.2f}",f"lambda_{lamda:.2f}",f"tau_{tension:.2f}","surf","eq")
                os.makedirs(surf_sim_dir_eq, exist_ok=True)

                surf_sim_dir_prod = os.path.join(base_dir, f"radius_{radius:.2f}",f"lambda_{lamda:.2f}",f"tau_{tension:.2f}","surf","prod")
                os.makedirs(surf_sim_dir_prod, exist_ok=True)

                with open("directories_surf.txt", "a") as directories:
                            directories.write(surf_sim_dir+"\n")
                
                with open("directories_surf_eq.txt", "a") as directories:
                            directories.write(surf_sim_dir_eq+"\n")

                with open("directories_surf_prod.txt", "a") as directories:
                        directories.write(surf_sim_dir_prod+"\n")
                ...
                # q_output_file = os.path.join(surf_sim_dir, "topol.q")
                # membrane_system.write_to_file(q_output_file)

                # top_file = os.path.join(surf_sim_dir, "top.top")
                # with open(top_file, "w") as f:
                #     f.write(f"topol.q 9\n")

                # Create input.dts file with simulation parametersss
                surf_input_file = os.path.join(surf_sim_dir, "input.dts")

                with open(surf_input_file, "w") as f:
                    f.write(f"""Integrator_Type = MC_Simulation
Min_Max_Lenghts = 1 3
MinfaceAngle = -0.5
Temperature = 1 0
Box_Centering_F = 0
...
TotalAreaCoupling = HarmonicPotential 0 0.4
...
;OpenEdgeEvolution =   EvolutionWithConstantVertex  5 0.2
...
;1           Pro1   20   10    10    5    0.0    0.0     0      10        0     0     0.0
;GenerateInclusions
...
;1    1    1   2  4   -4                  
""")
                surf_input_file_prod = os.path.join(surf_sim_dir_prod, "input.dts")

                with open(surf_input_file_prod, "w") as f:
                    f.write(f"""Integrator_Type = MC_Simulation
Min_Max_Lenghts = 1 3
MinfaceAngle = -0.5
Temperature = 1 0
...
EnergyMethod = FreeDTS1.0_FF
Kappa = 20  0 0
Edge_Parameters = {lamda:.2f} 0 0
VertexArea = 0 0.4 0 0
TimeSeriesData_Period = 100
VertexPositionIntegrator = MetropolisAlgorithm 1 1 0.05
AlexanderMove = MetropolisAlgorithm 1
InclusionPoseIntegrator = MetropolisAlgorithm   1 1
Dynamic_Box =   IsotropicFrameTension 5 {tension:.2f} XY
OpenEdgeEvolution =   EvolutionWithConstantVertex  5 0.2
VisualizationFormat = VTUFileFormat VTU_F 1000
NonbinaryTrajectory = TSI TrajTSI 1000
Restart_Period = 1000
